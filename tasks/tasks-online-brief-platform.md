...
- [ ] **5.0 List-calendar + schema migration + polish + retire static dashboard**
  ...
  - [ ] 5.4 Manually trigger both briefs via the `Run brief` workflow on the feature branch's Vercel preview deployment URL; verify the Slack message + CSV thread reply still arrive correctly. *(Manual user task — pending.)*
